...
// TestDatabaseDeletedViaNestedRoute guards the delete half of issue #30: the
// flat DELETE /databases/{id} route 302s on the API, so the resource must delete
// through the cluster-nested route. The fake serves DELETE only at the nested
...
// so the import must carry it as "bucket_id:key_id". This is the only route
// back for keys orphaned by the broken delete endpoint (issue #31).
