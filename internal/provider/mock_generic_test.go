...
type crudSpec struct {
	...
	// API that returns a different id on create than it stores — e.g. dev's
	// POST /databases/clusters/{cid}/databases returns the cluster id, not the
	// new schema id (see resource_database.go / issue #30).
	...
	// resource must use a parent-nested route — e.g. dev's
	// DELETE /databases/clusters/{cid}/databases/{id} (flat /databases/{id}
	// 302s; see issue #30). It still deletes by the "{id}" path value.
