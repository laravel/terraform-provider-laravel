...
# what each resource accepts. If the dev API rejects a region / type / size,
# tweak it here.
# ===========================================================================
...
# BLOCKED by an API bug -- off by default so a plain `apply` stays green.
# Enable with -var enable_api_blocked=true to re-test it. See README.
...
# BLOCKED by the same API bug as background_process, above.
...
resource "laravel_cloud_database_cluster" "cluster" {
  ...
  region = "us-east-2"
  # The dev API requires the full config_schema for laravel_mysql_84:
